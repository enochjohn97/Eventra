  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (result.success) {
        ...
        setTimeout(() => {
          const redirectUrl =
            result.redirect || "/admin/pages/adminDashboard.html";
        ...
        const errorElement = result.message?.toLowerCase().includes("username")
          ? "usernameError"
          : "passwordError";
        showError(
          errorElement,
  ...
  async function initSlider() {
    ...
    try {
      ...
      if (data.success && data.events && data.events.length > 0) {
        ...
        // Inject images
        sliderContainer.innerHTML = eventsWithImages
          .map((event, index) => {
