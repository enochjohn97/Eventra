document.addEventListener("DOMContentLoaded", () => {
  ...
  const intent =
    roleParam ||
    intentParam ||
    document.body.getAttribute("data-intent") ||
    "client";
  ...
  if (intent === "admin") {
    ...
    document.title =
      (intent === "client" ? "Client" : "User") + " Registration - Eventra";
    if (signupTitle)
      signupTitle.textContent =
        intent === "client" ? "Client Registration" : "Create Account";
    if (signupButton)
      signupButton.textContent =
        intent === "client" ? "Create Client Account" : "Sign Up";
  ...
  if (signupForm) {
    ...
    try {
      localStorage.removeItem("form_state_signupForm");
    } catch (e) {}
    ...
    signupForm.addEventListener("submit", (e) => {
      ...
      if (intent === "client" && !businessName) {
        showError("businessNameError", "Business name is required.");
        isValid = false;
      } else if (
        intent === "client" &&
        !/^(?=.*[A-Za-z])[A-Za-z0-9][A-Za-z0-9 .,&'’()_-]{1,99}$/.test(
          businessName,
        )
      ) {
        showError(
          "businessNameError",
          "Enter a valid business name using letters, numbers, spaces, and basic punctuation.",
        );
        isValid = false;
      }
  ...
  function showError(elementId, message) {
    ...
    if (inputElement) {
      ...
      if (window.eventraSetFieldState)
        window.eventraSetFieldState(inputElement, "error", message);
    }
  }
    ...
    try {
      ...
      if (data.success) {
        ...
        setTimeout(() => {
          const loginUrl =
            intent === "admin"
              ? "clientLogin.html?role=admin"
              : `clientLogin.html?role=${intent}`;
          ...
          if (verifyResult.success) {
            ...
            setTimeout(() => {
              window.location.href =
                verifyResult.redirect || "clientDashboard.html";
            }, 2100);
          } else {
            Swal.fire({
              icon: "error",
