...
function showOTPModal(email, phone, onVerified, onCancel) {
  const reference =
    "PAY-" + Date.now().toString(36) + Math.random().toString(36).substr(2, 5);
  ...
  if (!email && !phone) {
    Swal.fire(
      "Error",
      "No contact information available for verification.",
      "error",
    );
    ...
    if (result.isConfirmed) {
      ...
      Swal.fire({
        ...
        html: `<p style="font-size: 0.9em; color: #666; margin-bottom: 1rem;">${result.value.message}</p>
                       <input id="otpCode" class="swal2-input" type="text" inputmode="numeric" pattern="[0-9]{6}" placeholder="6-digit code" autocomplete="one-time-code" maxlength="6" style="text-align: center; letter-spacing: 0.4em; font-size: 1.5em; font-weight: bold; width: 280px; max-width: 100%; box-sizing: border-box;">
