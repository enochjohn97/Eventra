  ...
  try {
    ...
    if (result.success) {
      ...
      profileAvatars.forEach((avatar) => {
        const avatarUrl =
          user.profile_pic ||
          `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name || user.business_name || "User")}&background=random&color=fff`;
        ...
        if (user.verification_status === "verified") {
          ...
          if (!localStorage.getItem("approved_notification_shown_" + user.id)) {
            ...
            localStorage.setItem(
              "approved_notification_shown_" + user.id,
              "true",
            );
          }
        } else {
...
function displayStatsCards(stats) {
  ...
  if (ticketsEl)
    ticketsEl.textContent =
      stats.total_tickets !== undefined ? stats.total_tickets : 0;
  if (usersEl)
    usersEl.textContent =
      stats.total_users !== undefined ? stats.total_users : 0;
  if (mediaEl)
    mediaEl.textContent =
      stats.total_media !== undefined ? stats.total_media : 0;
}
...
async function loadRecentTickets(attendees) {
  ...
  if (!attendees || attendees.length === 0) {
    salesList.innerHTML =
      '<p style="text-align: center; color: var(--client-text-muted); padding: 2rem;">No ticket sales yet.</p>';
    return;
  }

  salesList.innerHTML = attendees
    .map((attendee) => {
      let paymentMethod = "Paystack";
      try {
        if (
          attendee.paystack_response &&
          typeof attendee.paystack_response === "string"
        ) {
          const parsed = JSON.parse(attendee.paystack_response);
          if (parsed.data && parsed.data.channel)
            paymentMethod = parsed.data.channel.toUpperCase();
        }
      } catch (e) {}
